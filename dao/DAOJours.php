...
use BWB\Framework\mvc\models\Jours;
use PDO;
/* 
...
/* ____________________Crud methods____________________*/
 ...
 var_dump($array);
		...
		$Foodtruck = $array[1];
				...
				$sql = "INSERT INTO Jours (Events_id,Foodtrucks_id) VALUES('". $eventId . "','" . $Foodtruck . "')";
				var_dump($sql);
...
//		
	...
	public function retrieve ($id){
		...
		$sql = "SELECT * FROM Jours WHERE Events_id= '$id' " ;
//                var_dump($sql);
...
//            var_dump($array);
              ...
              $id = $array['id'];
                $role = $array['role'];
                try {
                    $sql = "UPDATE Jours SET role = '" . $role ."' WHERE id = ". $id;
//                var_dump($sql);
	...
	public function delete ($id){
		...
		$sql = "DELETE FROM Jours WHERE id= " . $id;
		$this->getPdo()->query($sql);
	}
...
//                var_dump($results);
		foreach($results as $result){
			$entity = new Jours();
			$entity->setId_jours($result['id_jours']);
			$entity->setNom_jour($result['nom_jour']);
